...
def pad_nearest(x, target_h, target_w):
    """
    Nearest-neighbor upsampling on LAST TWO dimensions (H, W).

    x: tensor of shape (..., H, W)
    target_h, target_w: desired spatial size
    """
    *prefix, H, W = x.shape

    # Flatten prefix dims into batch for interpolation
    ...
    # Restore original prefix dimensions + new HW
    return y.reshape(*prefix, target_h, target_w)


def depad_nearest(x, target_h, target_w):
    """
    Reverse nearest-neighbor padding on LAST TWO dimensions (H, W).
    
    x: tensor of shape (..., H_up, W_up)
    target_h, target_w: original H, W before padding
    """
    H_up, W_up = x.shape[-2], x.shape[-1]

    factor_h = H_up // target_h
    factor_w = W_up // target_w

    return x[..., ::factor_h, ::factor_w]
