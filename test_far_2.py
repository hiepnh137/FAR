...
def predict_loop(name, data, seq_len, pred_len, seeds=None, min_data=None, max_data=None):
    ...
    for i in range(N):
        ...
        out_np = out_np * (max_data - min_data) + min_data
...
if __name__ == "__main__":
    ...
    raw_data, data, min_data, max_data = read_data_2(data_path_dict[dataset_name])
    # data = data[100:105]
    # raw_data = raw_data[100:105]
    print('data shape: ', data.shape)  # N x T x H x W
    seq_len = 6
    pred_len = 6
    pred = predict_loop(dataset_name, data, seq_len, pred_len, min_data=min_data, max_data=max_data)
